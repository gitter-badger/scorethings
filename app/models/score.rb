class Score
  ...
  belongs_to :user
  belongs_to :scored_thing

  embeds_many :old_points

  search_in scored_thing: [:title, :description, :instance_of_values], user: [:username], criterion: [:name]
  ...
  validates_presence_of :scored_thing, :user, :points
